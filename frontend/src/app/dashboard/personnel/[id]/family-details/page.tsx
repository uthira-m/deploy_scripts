...
interface FamilyDetail {
  id: number;
  relationship_type: 'father' | 'mother' | 'spouse' | 'child' | 'sister' | 'brother';
  name: string;
  dob?: string;
...
const FamilyDetailsPage = () => {
  ...
  const handleDeleteFamilyDetail = (detailId: number) => {
    setConfirmModal({
      isOpen: true,
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          notifyError(err.message || 'Failed to delete family member');
        }
      },
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingDetail) {
        ...
        await personnelService.createFamilyDetail(parseInt(personnelId), formData);
        ...
        {user?.role !== 'personnel' && (
          <div className="absolute top-3 right-3 flex ">
            <button
              ...
              className="px-3 py-1.5 hover:bg-slate-600 text-white text-sm rounded-lg transition-colors duration-200"
              title="Edit"
            >
            ...
            <button
              ...
              className="px-3 py-1.5  hover:bg-red-600 text-white text-sm rounded-lg transition-colors duration-200"
              title="Delete"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="bg-slate-900 px-6 py-4 border-b border-slate-800">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-2xl font-bold text-white">Family Details Management</h1>
            </div>
            {familyDetails.length > 0 && user?.role !== 'personnel' && (
              <button
            ...
            <div className="text-center py-16">
              <div className="max-w-md mx-auto">
                ...
                <p className="text-slate-400 mb-8 leading-relaxed">
                  Add family member details to keep track of important information including contact details, documents, and personal information.
                </p>
                {user?.role !== 'personnel' && (
                  <button
                    onClick={handleAddFamilyDetail}
